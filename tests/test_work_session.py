...
def test_get_vacancies():
    ...
    ws = WorkSession(dao)
    ws.get_vacancies(SessionActions.SAVE, 'hh', 'Python', 3, 2)
    with open(PATH_TO_TEST_JSON) as file:
        vacancies = file.readlines()
        assert isinstance(vacancies, list)
        assert isinstance(vacancies[0][0], str)

    vacancies = ws.get_vacancies(SessionActions.ALL, 'hh', 'Python', 3, 2)
    assert isinstance(vacancies, list)
    vacancies = ws.get_vacancies(SessionActions.TOP, 'hh', 'Python', 3, 2)
    assert isinstance(vacancies, list)
